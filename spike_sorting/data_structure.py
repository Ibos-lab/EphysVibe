...
def sort_data_trial(clusters, spiketimes, start_trials, end_trial, real_strobes, filtered_timestamps, spiketimes_clusters_id,full_word, LFP_ds, eyes_ds):
    ...
    for trial_i in range(len(start_trials)): # iterate over trials
        ...
        # define trial masks
        sp_mask = np.logical_and(spiketimes >= start_trials[trial_i] , spiketimes < end_trial[trial_i])
        events_mask = np.logical_and(real_strobes >= start_trials[trial_i] , real_strobes <= end_trial[trial_i])
        lfp_mask = np.logical_and(filtered_timestamps >= start_trials[trial_i] , filtered_timestamps <= end_trial[trial_i])
        # split spikes
        sp_trial = spiketimes[sp_mask]
        id_clusters = spiketimes_clusters_id[sp_mask] # to which neuron correspond each spike

        # split code numbers
        code_numbers.append(full_word[events_mask]) # all trials have to start and end with the same codes
        # split code times
...
def build_data_structure(clusters, times, code_numbers, code_times,eyes_sample, lfp_sample, timestamps):
